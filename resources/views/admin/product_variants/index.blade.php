        ...
        <div class="tile">
            <div class="tile-body">
                <div class="row element-button">
                    <div class="col-sm-2">
                        <a class="btn btn-add btn-sm" href="{{ route('admin.product-variants.create') }}" title="Thêm"><i class="fas fa-plus"></i> Tạo mới biến thể</a>
                    </div>
                    <div class="col-sm-2">
                ...
                <table class="table table-hover table-bordered" id="sampleTable">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $variant->sale_price ? number_format($variant->sale_price, 0, ',', '.') : 'N/A' }} đ</td>
                            <td>{{ $variant->quantity }}</td>
                            <td>
                                @if ($variant->images)
                                    <img src="{{ asset('storage/' . $variant->images) }}" alt="{{ $variant->product->name ?? 'N/A' }}" width="100px;">
                                @else
                                    <span>Không có ảnh</span>
                                @endif
                            </td>
                            <td>
                                <a href="{{ route('admin.product-variants.show', $variant->id) }}" class="btn btn-primary btn-sm" title="Xem chi tiết">
                                    <i class="fas fa-eye"></i>
                                </a>
                                <a href="{{ route('admin.product-variants.edit', $variant->id) }}" class="btn btn-primary btn-sm edit" title="Sửa">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <form action="{{ route('admin.product-variants.destroy', $variant->id) }}" method="POST" style="display:inline;" onsubmit="return confirm('Bạn có chắc muốn xóa biến thể này?');">
                                    @csrf
                                    @method('DELETE')
